...
- [11. Point Model](#11-point-model)
  ...
  - [11.6 BACnet-side point names, and what actually twins with what](#116-bacnet-side-point-names-and-what-actually-twins-with-what)
  - [11.7 How a P2 point and a BACnet object are joined](#117-how-a-p2-point-and-a-bacnet-object-are-joined)
...
- **Encoding.** Strings are TLVs — `<textType:u8> <textLen:u16 BE> <bytes>`, commonly rendered `01 00 <len> …` because the length rarely exceeds 255 and `textType` is usually `0x01`. **Read both fields; never test `textType == 0x01` and never read a one-byte length** (§8.1). Analog values are IEEE-754 single-precision big-endian floats; command priority rides a scope tag. Bodies are ordered, positionally-typed field structures (ASDUs).
...
| `0x00AC` | **not supported** — the function code is unused on this panel, **or is specific to a different firmware revision** | wire behaviour and the vendor catalog agree, **and the revision case is now confirmed from the device-side definitions** — see below | [W][D][S] |
| `0x0002` | **invalid command** — the command is not valid for the addressed object (seen on `POINT_CMD_ALARM 0x0244` and on `CMD_ALARM_DISABLE 0x0247` addressed to a point name that does not exist; the documented example is commanding a non-virtual LDI or LAI point). **Also returned for an over-length `0x0136 P2_ROUTE`**, see below | wire behaviour, the vendor catalog and the device-side definitions agree | [W][D][S] |
...
Of the **457 operations** that have a request and/or response structure:

| | operations |
|---|---:|
| **Decodable** from widths and tag maps this document pins | **457 (100%)** |
| Blocked on an unpinned width or CHOICE | **0** |

Two of the 457 arrived late and by a different route. The opcode-to-structure
mapping is otherwise generated by matching the opcode's own name against
`AP2_<name>_Request` / `_Response`, and two operations that rule can never
reach were added on wire evidence: `0x4200`, whose opcode is named
...
reached its ceiling.** The first version of this paragraph said so honestly: forty-four
...
> **The converse does not hold, and reading it as though it did is a real defect.** *Having* a default entry implies enumerated states; **lacking one does not imply analog.** `LFMSSL` (22) and `LFMSSP` (23) have **no** `-22`/`-23` entry in the library and are **digital four-state** points — the fast/medium/slow/stop families this section lists as digital-only, whose structures carry a `state_text_table` like every other enumerated type. A reader who infers the analog/digital split from the presence of a default enumeration classifies a four-state motor-speed point as an analog value. `p2_scanner.py` did exactly that until 2026-09-21; it now carries an explicit exception table for the two. [S][I]

> So there are **seven** point types with more than two states — `LOOAP`, `LOOAL`, `LFSSL`, `LFSSP`, `LENUM`, `LFMSSL`, `LFMSSP` — and a consumer mapping them onto BACnet needs a multi-state object for all seven. The state **count** for the last two is derived from the mnemonic and agrees with the subpoint table above (the pulsed variant carries one DO per state, 4 DO = 4 states, exactly as `LFSSP` carries 3 DO for STOP/SLOW/FAST). Their state **text** is unknown: the vendor library supplies no default, so a panel carrying such a point supplies its own table and nothing here should guess it. **[OPEN]**
...
eighteen above were recovered from *panel* databases. A hundred and fifty-one further
...
**Twenty is a floor that did not move.** All nine dated states of that
supervisor were censused -- 151 capability documents, 144 panels, spanning
2020-04 to 2022-06. No twenty-first `<Services>` element appears, none of the
twenty is ever absent, no seventh `HardwareType`/`Platform` pair exists, and
no pair occurs only in an intermediate state. The set is open by construction
and stable in practice across two and a half years of one deployment. [S]
...
- **`<PanelBasics>` carries `<Platform>` and `<VersionNumber>`** beyond the
  ...
  across all 144 panels read from the supervisor databases --
  `PXME`/`PME` (75), `PXMP`/`PMP` (22), `EPXC`/`PXE` (21), `MECF`/`MCF` (9),
  `MCFP`/`MCA` (9), `PXCE`/`PCE` (8) -- so a decoder may treat either as
...
7. **CHOICE tag→arm assignment — ANSWERED for every CHOICE a decode depends
   ...
   *Now:* the numbering is read directly for **72 of the 73 CHOICEs**, complete
   ...
   register is at 457 of 457 with nothing blocked and no modelling assumption
